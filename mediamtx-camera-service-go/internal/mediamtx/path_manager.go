...
func (pm *pathManager) WaitForPathReady(ctx context.Context, name string, timeout time.Duration) error {
	...
	// Use centralized configuration for timeout and polling intervals
	configTimeout := time.Duration(pm.config.StreamReadiness.Timeout) * time.Second
	if timeout > 0 {
		configTimeout = timeout // Override with provided timeout if specified
	}

	// Get polling intervals from configuration
	initialInterval := time.Duration(pm.config.StreamReadiness.InitialCheckInterval) * time.Second
	checkInterval := time.Duration(pm.config.StreamReadiness.CheckInterval) * time.Second
	maxInterval := time.Duration(pm.config.StreamReadiness.MaxCheckInterval) * time.Second

	// Set defaults if configuration values are zero
	...
	for {
		...
		case <-ticker.C:
			...
			if time.Now().After(deadline) {
				...
				}).Warn("Path readiness timeout exceeded")
				return NewPathError(name, "wait_ready", fmt.Sprintf("path %s not ready within %v (checked %d times)", name, configTimeout, checkCount))
			}
			...
			if err != nil {
				...
				// Increase polling interval gradually to reduce API load
				pm.adjustPollingInterval(&currentInterval, checkInterval, maxInterval, &ticker)
				continue
			}
...
// adjustPollingInterval gradually increases polling interval to reduce API load
func (pm *pathManager) adjustPollingInterval(currentInterval *time.Duration, checkInterval, maxInterval time.Duration, ticker **time.Ticker) {
	// Increase interval gradually: 200ms -> 500ms -> 1s -> 2s
